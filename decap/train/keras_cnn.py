...
# 基本参数配置
height, width, n_len, n_class = 60, 200, 3, 14

def load_data(path='img/origin2'):
...
#    random.shuffle(fnames)
    ...
    for idx, fname in enumerate(fnames):
        ...
        label.append(fname.split('/')[-1].split('_')[0].lower())
    return data, label
...
if DEBUG:
    ...
    # 重新加载进行训练
    model_cnn = load_model('model/tgcode_cm.h5')
...
model_cnn.fit_generator(datagen.flow(x_train, y_train, batch_size=32), samples_per_epoch=len(x_train), epochs=5)
# model_cnn.fit(x_train, y_train, batch_size=32, epochs=3)
# 保存模型
model_cnn.save('model/tgcode_cm.h5')
